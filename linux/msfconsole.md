...
| migrate                                               | `migrate 568`는 Meterpreter 세션에서 `PID`(Process ID)가 568인 프로세스로 세션을 이동시키는 명령어입니다. 이 명령어를 통해 현재 세션을 더 적합한 프로세스로 이동함으로써 지속성을 유지하거나, 더 높은 권한을 가진 프로세스에서 실행되도록 할 수 있습니다. |
